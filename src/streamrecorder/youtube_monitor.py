...
class YouTubeMonitor:
    ...
    async def check_stream_status(self, channel: str) -> Optional[StreamInfo]:
        ...
        # Run yt-dlp in executor to not block event loop. Exceptions are allowed
        # to bubble to the monitor loop so transient extractor/network failures
        # do not get misclassified as "offline".
    ...
    async def _check_all_channels(self) -> List[MonitorEvent]:
        ...
        for channel in self.channels:
            ...
            try:
                ...
                logger.warning(f"YouTube check failed, keeping previous state: {e}")
