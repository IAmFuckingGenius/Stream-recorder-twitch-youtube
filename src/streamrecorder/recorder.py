...
class StreamRecorder:
    ...
    def _is_main_recording_candidate(self, path: Path) -> bool:
        """Return True only for files that can be direct yt-dlp outputs."""
        if not path.is_file():
            return False
        name = path.name
    ...
    async def record_stream(
        ...
        if temp_path.exists():
            ...
            for pattern in search_patterns:
                matching_files = [
                    f for f in self.temp_dir.glob(pattern)
                    if self._is_main_recording_candidate(f)
                ]
                if matching_files:
